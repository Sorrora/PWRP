...
AddEventHandler('esx_property:hasEnteredMarker', function(name, part, parking)
	...
	if part == 'ExteriorEntryPoint' then
		...
		-- Store the entered garage in the database
		TriggerServerEvent('esx_garage:setCurrentGarage', name)
	...
	if part == 'InteriorExitPoint' then
		ESX.TriggerServerCallback('esx_garage:getCurrentGarage', function(currentGarage)
